...
fn test_valid_full_json_exits_zero_with_no_stdout() {
    ...
    cargo_bin_cmd!("keyjey").write_stdin(json).assert().success();
}
...
fn test_config_flag_with_valid_toml_exits_zero() {
    ...
    keyjey()
        ...
        .assert()
        .success()
        // sample_starship.toml has lines = ["$keyjey.model $git_branch", "$keyjey.cost"]
...
fn test_no_local_config_falls_through_to_global_or_default() {
    ...
    //   - Step 4: KeyjeyConfig::default() if no global config exists either.
    ...
    let json = std::fs::read_to_string("tests/fixtures/sample_input_minimal.json").unwrap();
    keyjey().write_stdin(json).assert().success();
}
...
fn test_model_renders_display_name_to_stdout() {
    let json = std::fs::read_to_string("tests/fixtures/sample_input_full.json").unwrap();
    // model_only.toml: lines = ["$keyjey.model"], no style → plain text "Opus"
    keyjey()
...
fn test_two_row_layout_produces_newline_separated_output() {
    let json = std::fs::read_to_string("tests/fixtures/sample_input_full.json").unwrap();
    // two_rows.toml: lines = ["$keyjey.model", "$keyjey.model"]
    let output = keyjey()
...
fn test_passthrough_tokens_skipped_silently() {
    ...
    let json = r#"{"session_id":"test","cwd":"/tmp","transcript_path":"/tmp/t.jsonl","version":"1.0","exceeds_200k_tokens":false,"model":{"id":"test","display_name":"Test"},"workspace":{"current_dir":"/nonexistent_keyjey_test_path_12345","project_dir":"/tmp"},"output_style":{"name":"default"},"cost":{"total_cost_usd":0.0}}"#;
...
fn test_missing_model_logs_warning_to_stderr() {
    ...
    keyjey()
        ...
        .success()
        .stdout("")
        .stderr(predicate::str::contains("keyjey.model"));
}

#[test]
fn test_no_lines_config_produces_empty_stdout() {
    let json = std::fs::read_to_string("tests/fixtures/sample_input_full.json").unwrap();
    // empty_keyjey.toml: [keyjey] with no lines key → cfg.lines is None → no output
    keyjey()
        .args(["--config", "tests/fixtures/empty_keyjey.toml"])
        .write_stdin(json)
        .assert()
        .success()
        .stdout("");
...
fn test_vim_mode_subfield_renders_identically() {
    let json = std::fs::read_to_string("tests/fixtures/sample_input_full.json").unwrap();
    // vim_mode_subfield.toml: lines = ["$keyjey.vim.mode"] → same as $keyjey.vim
    keyjey()
...
fn test_agent_name_subfield_renders_identically() {
    let json = std::fs::read_to_string("tests/fixtures/sample_input_full.json").unwrap();
    // agent_name_subfield.toml: lines = ["$keyjey.agent.name"] → same as $keyjey.agent
    keyjey()
...
// ── Story 3.1: keyjey explain integration tests ────────────────────────────
...
fn test_explain_with_stdin_json_shows_module_names() {
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(
        stdout.contains("keyjey.model"),
        "expected 'keyjey.model' in explain output: {stdout}"
    );
}
...
fn test_explain_no_stdin_uses_embedded_fallback() {
    ...
    let output = cargo_bin_cmd!("keyjey").args(["explain"]).output().unwrap();
    assert!(output.status.success());
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(
        stdout.contains("keyjey.model"),
...
fn test_explain_shows_warning_for_disabled_module() {
    ...
    let output = cargo_bin_cmd!("keyjey")
    ...
    assert!(
        stdout.contains("[keyjey.model]"),
        "expected specific section '[keyjey.model]' in remediation hint: {stdout}"
    );
}
...
fn test_native_renders_alongside_passthrough_not_installed() {
    // sample_starship.toml: lines = ["$keyjey.model $git_branch", "$keyjey.cost"]
    // Native $keyjey.model renders "Opus" regardless of whether starship is present.
...
// ── Story 4.2: Cache and KEYJEY_* env var integration tests ────────────────

#[test]
fn test_passthrough_env_vars_injected_via_keyjey_model() {
    // Create a fake starship script that echoes $KEYJEY_MODEL to stdout.
    // Uses .env("PATH", ...) on the keyjey subprocess rather than mutating
    // the test process's global PATH — safe for parallel test execution.
    use std::fs;
    #[cfg(unix)]
    use std::os::unix::fs::PermissionsExt;

    let dir = std::env::temp_dir().join("keyjey_inttest_keyjey_env");
    fs::create_dir_all(&dir).unwrap();
    let script = dir.join("starship");
    fs::write(&script, "#!/bin/sh\nprintf '%s' \"$KEYJEY_MODEL\"\n").unwrap();
    #[cfg(unix)]
    fs::set_permissions(&script, fs::Permissions::from_mode(0o755)).unwrap();

    let json = r#"{"session_id":"test","cwd":"/tmp","transcript_path":"/tmp/keyjey_inttest_tp.jsonl","version":"1.0","exceeds_200k_tokens":false,"model":{"id":"claude-opus-4-6","display_name":"IntTestModel"},"workspace":{"current_dir":"/tmp","project_dir":"/tmp"},"output_style":{"name":"default"},"cost":{"total_cost_usd":0.0}}"#;
    ...
    let _ = fs::remove_file("/tmp/keyjey/keyjey_inttest_tp-starship-git_branch");
    let _ = fs::remove_dir("/tmp/keyjey");

    assert!(result.status.success());
    let stdout = String::from_utf8_lossy(&result.stdout);
    assert!(
        stdout.contains("IntTestModel"),
        "expected KEYJEY_MODEL value 'IntTestModel' in passthrough output: {stdout:?}"
    );
}

#[test]
fn test_cache_hit_does_not_spawn_subprocess() {
    // Pre-write a cache file at the expected path, then verify keyjey returns it
    // without needing starship installed. Uses .env("PATH", ...) on the keyjey
    // subprocess to block starship lookup — safe for parallel test execution.
    use std::fs;

    // Cache path: {dirname(transcript_path)}/keyjey/{stem}-starship-{module}
    // transcript_path = "/tmp/keyjey_cache_inttest.jsonl"
    // → /tmp/keyjey/keyjey_cache_inttest-starship-git_branch
    let cache_dir = std::path::Path::new("/tmp/keyjey");
    fs::create_dir_all(cache_dir).unwrap();
    let cache_file = cache_dir.join("keyjey_cache_inttest-starship-git_branch");
    fs::write(&cache_file, "cached-branch-value").unwrap();

    let json = r#"{"session_id":"test","cwd":"/tmp","transcript_path":"/tmp/keyjey_cache_inttest.jsonl","version":"1.0","exceeds_200k_tokens":false,"model":{"id":"claude-opus-4-6","display_name":"Opus"},"workspace":{"current_dir":"/tmp","project_dir":"/tmp"},"output_style":{"name":"default"},"cost":{"total_cost_usd":0.0}}"#;

    let result = keyjey()
        .args(["--config", "tests/fixtures/passthrough_only.toml"])
        // Block starship subprocess to prove the cached value is used instead
        .env("PATH", "/nonexistent_keyjey_test_dir_42")
        .write_stdin(json)
        .output()
        .unwrap();
...
fn test_literal_text_in_lines_preserved() {
    let json = std::fs::read_to_string("tests/fixtures/sample_input_full.json").unwrap();
    // literal_text.toml: lines = ["in: $keyjey.context_window.total_input_tokens"]
...
fn test_format_field_line_break_produces_two_rows() {
    let json = std::fs::read_to_string("tests/fixtures/sample_input_full.json").unwrap();
    // format_line_break.toml: format = "$keyjey.model$line_break$keyjey.model"
    let output = keyjey()
