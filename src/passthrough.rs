...
//! Story 4.2: adds 5s file cache (`cache.rs`) and KEYJEY_* environment variable injection.
...
/// - Injects all 9 KEYJEY_* environment variables into the subprocess environment.
pub fn render_passthrough(name: &str, ctx: &crate::context::Context) -> Option<String> {
    ...
    if cwd.is_none() {
        tracing::warn!(
            "passthrough: no CWD available for `{name}` — subprocess inherits keyjey's cwd"
        );
    }
    ...
    // KEYJEY_* environment variable injection (all 9 — empty string for None fields)
    cmd.env(
        "KEYJEY_MODEL",
        san(ctx
            .model
    ...
    cmd.env("KEYJEY_CWD", san(cwd.unwrap_or("")));
...
mod tests {
    ...
    fn test_render_passthrough_returns_none_for_nonexistent_module() {
        ...
        let result = render_passthrough("__keyjey_nonexistent_xyz__", &Context::default());
    ...
    fn test_render_passthrough_returns_none_on_nonzero_exit() {
        ...
        let dir = std::env::temp_dir().join("keyjey_test_nonzero");
    ...
    fn test_render_passthrough_returns_none_silently_when_starship_missing() {
        ...
        let _guard = PATH_MUTEX.lock().unwrap();
        let original = std::env::var("PATH").unwrap_or_default();
        unsafe { std::env::set_var("PATH", "/nonexistent_keyjey_test_dir") };
        let result = render_passthrough("directory", &Context::default());
        unsafe { std::env::set_var("PATH", &original) };
        assert!(result.is_none());
    }
    ...
    #[test]
    fn test_render_passthrough_injects_keyjey_model_env_var() {
        use std::fs;
        #[cfg(unix)]
        use std::os::unix::fs::PermissionsExt;

        let dir = std::env::temp_dir().join("keyjey_test_keyjey_env");
        fs::create_dir_all(&dir).unwrap();
        let script = dir.join("starship");
        // Script: print KEYJEY_MODEL env var, exit 0
