//! `keyjey uninstall` — removes binary, settings.json statusline entry, and cache dirs.
//! Intentionally leaves `[keyjey.*]` sections in `starship.toml` intact (FR44).
...
fn remove_cache_directories(home: &std::path::Path) {
    ...
    // Walk one level deep: ~/.claude/projects/{hash}/keyjey/
    ...
    for entry in entries.flatten() {
        let cache_dir = entry.path().join("keyjey");
    ...
    if removed > 0 {
        println!(
            "Removed {removed} keyjey cache director{}.",
            if removed == 1 { "y" } else { "ies" }
        );
    } else {
        println!("No keyjey cache directories found — skipping.");
    }
}
...
mod tests {
    ...
    fn test_remove_binary_present() {
        with_tempdir(|home| {
            ...
            let local_path = local_bin.join("keyjey");
            std::fs::write(&local_path, b"fake binary").unwrap();

            let cargo_bin = home.join(".cargo/bin");
            std::fs::create_dir_all(&cargo_bin).unwrap();
            let cargo_path = cargo_bin.join("keyjey");
    ...
    fn test_remove_statusline_present() {
        with_tempdir(|home| {
            ...
            std::fs::write(
                &settings_path,
                r#"{"statusline":"keyjey","otherKey":"value"}"#,
            )
            .unwrap();
            remove_statusline_from_settings(home);
    ...
    fn test_remove_cache_dirs_present() {
        with_tempdir(|home| {
            ...
            let cache_dir = hash_dir.join("keyjey");
            std::fs::create_dir_all(&cache_dir).unwrap();
            std::fs::write(cache_dir.join("transcript-starship-git_branch"), b"data").unwrap();
            assert!(cache_dir.exists());
            remove_cache_directories(home);
            assert!(!cache_dir.exists(), "keyjey cache dir should be removed");
        });
    }
