...
#[derive(Parser, Debug)]
#[command(name = "keyjey", about = "KeyJey Claude Code statusline renderer")]
struct Cli {
...
enum Commands {
    ...
    Explain,
    /// Remove keyjey binary and settings.json entry.
    Uninstall,
}
...
fn main() {
    ...
    match cli.command {
        Some(Commands::Explain) => {
            let output = keyjey::explain::run(cli.config.as_deref());
            if !output.is_empty() {
                println!("{output}");
            }
        }
        Some(Commands::Uninstall) => {
            keyjey::uninstall::run();
        }
        None => {
            let ctx = match keyjey::context::from_stdin() {
                Ok(ctx) => ctx,
                Err(e) => {
                    tracing::error!("keyjey: failed to parse Claude Code session JSON: {e}");
                    std::process::exit(1);
                }
            };
            ...
            let cfg = match keyjey::config::discover_and_load(
                workspace_dir,
                cli.config.as_deref().and_then(|p| p.to_str()),
            ) {
                Ok(cfg) => cfg,
                Err(e) => {
                    tracing::error!("keyjey: failed to load config: {e}");
                    std::process::exit(1);
                }
            };
            ...
            if cfg.format.is_some() || !lines.is_empty() {
                let output = keyjey::renderer::render(lines, &ctx, &cfg);
                if !output.is_empty() {
                    println!("{output}");
                }
            }
