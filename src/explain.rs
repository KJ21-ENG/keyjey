//! `keyjey explain` subcommand — shows each native module's rendered value and config source.

const SAMPLE_CONTEXT: &str = include_str!("sample_context.json");
const SAMPLE_CONTEXT_PATH: &str = ".config/keyjey/sample-context.json";
const LEGACY_SAMPLE_CONTEXT_PATH: &str = ".config/keyjey/sample-context.json";
...
pub fn run(config_override: Option<&std::path::Path>) -> String {
    ...
    const CFG_W: usize = 24; // "[keyjey.context_window]" = 24 chars

    let mut lines = Vec::new();
    lines.push(format!("keyjey explain — using config: {source}"));
...
fn load_context() -> (crate::context::Context, Vec<String>) {
    ...
    if !std::io::stdin().is_terminal() {
        match crate::context::from_reader(std::io::stdin()) {
            ...
            Err(e) => {
                tracing::warn!(
                    "keyjey explain: failed to parse stdin JSON: {e} — falling back to sample context"
                );
            }
        }
    }

    // 2. Try ~/.config/keyjey/sample-context.json, then legacy keyjey path.
    if let Ok(home) = std::env::var("HOME") {
        ...
        if sample_path.exists() {
            ...
            let legacy_sample_path = std::path::Path::new(&home).join(LEGACY_SAMPLE_CONTEXT_PATH);
            if legacy_sample_path.exists()
                && let Ok(content) = std::fs::read_to_string(&legacy_sample_path)
                && let Ok(ctx) = serde_json::from_str(&content)
            {
                return (ctx, notes);
            }
...
fn strip_namespace(name: &str) -> &str {
    name.strip_prefix("keyjey.")
        .or_else(|| name.strip_prefix("keyjey."))
        .unwrap_or(name)
}

fn is_disabled(name: &str, cfg: &crate::config::KeyjeyConfig) -> bool {
    let top = strip_namespace(name);
...
fn error_hint_for(
    ...
    if is_disabled(name, cfg) {
        return (
            ...
            format!("Remove `disabled = true` from the [keyjey.{segment}] section in your config."),
        );
    }
    match segment {
        "model" => (
            "model data absent from Claude Code context".into(),
            "Ensure Claude Code is running and keyjey is invoked via the \"statusLine\" key in ~/.claude/settings.json.".into(),
        ),
        "cost" => (
        ...
        "usage_limits" => {
            ...
            // interactive `keyjey explain` command but must NOT be called from the
            ...
            match crate::platform::get_oauth_token() {
                Err(msg) if msg.contains("credentials not found") => (
                    ...
                    "Authenticate by opening Claude Code and completing the login flow, then run `keyjey explain` again.".into(),
                ),
                Ok(_) => (
                    "usage_limits returned no data — credential present but API fetch failed".into(),
                    "Your Claude Code token may have expired. Re-authenticate by opening Claude Code and completing the login flow, then run `keyjey explain` again.".into(),
                ),
                Err(_) => (
                    "usage_limits returned no data — credential appears malformed or tool unavailable".into(),
                    "Re-authenticate by opening Claude Code and completing the login flow, then run `keyjey explain` again.".into(),
                ),
            }
        }
        _ => (
            "module returned no value".into(),
            "Check keyjey configuration and ensure Claude Code is running.".into(),
        ),
    }
}

fn config_section_for(module_name: &str, cfg: &crate::config::KeyjeyConfig) -> &'static str {
    let top = strip_namespace(module_name);
    let segment = top.split('.').next().unwrap_or(top);
    match segment {
        "model" if cfg.model.is_some() => "[keyjey.model]",
        "cost" if cfg.cost.is_some() => "[keyjey.cost]",
        "context_bar" if cfg.context_bar.is_some() => "[keyjey.context_bar]",
        "context_window" if cfg.context_window.is_some() => "[keyjey.context_window]",
        "vim" if cfg.vim.is_some() => "[keyjey.vim]",
        "agent" if cfg.agent.is_some() => "[keyjey.agent]",
        "cwd" | "session_id" | "transcript_path" | "version" | "output_style"
            if cfg.session.is_some() =>
        {
            "[keyjey.session]"
        }
        "workspace" if cfg.workspace.is_some() => "[keyjey.workspace]",
        "usage_limits" if cfg.usage_limits.is_some() => "[keyjey.usage_limits]",
        _ => "(default)",
    }
}
...
mod tests {
    use super::*;
    use crate::config::{KeyjeyConfig, ModelConfig};
    ...
    fn test_run_contains_all_module_names() {
        let output = run(None);
        assert!(
            output.contains("keyjey.model"),
            "expected 'keyjey.model' in output"
        );
        assert!(
            output.contains("keyjey.cost"),
            "expected 'keyjey.cost' in output"
        );
        assert!(
            output.contains("keyjey.context_bar"),
            "expected 'keyjey.context_bar' in output"
        );
        assert!(
            output.contains("keyjey.vim"),
            "expected 'keyjey.vim' in output"
        );
    }
    ...
    fn test_config_section_for_model_with_config() {
        let mut cfg = KeyjeyConfig::default();
        cfg.model = Some(crate::config::ModelConfig::default());
        assert_eq!(config_section_for("keyjey.model", &cfg), "[keyjey.model]");
    }

    #[test]
    fn test_config_section_for_model_without_config() {
        let cfg = KeyjeyConfig::default();
        assert_eq!(config_section_for("keyjey.model", &cfg), "(default)");
    }
    ...
    fn test_load_with_source_respects_workspace_dir() {
        ...
        // Should fall through to global (starship.toml or keyjey.toml) or default without panicking
        assert!(
            matches!(
                result.source,
    ...
    fn test_run_output_shows_sample_model_value() {
        ...
        let cfg = KeyjeyConfig::default();
        let value = crate::modules::render_module("keyjey.model", &ctx, &cfg);
    ...
    fn test_run_with_valid_context_shows_model_in_explain_column() {
        ...
        let cfg = KeyjeyConfig::default();
        let value = crate::modules::render_module("keyjey.model", &model_ctx, &cfg);
    ...
    fn test_run_shows_warning_indicator_for_none_module() {
        // run() loads the embedded sample context — keyjey.context_window.exceeds_200k
        // returns None because sample context value is below 200k threshold.
        let output = run(None);
        assert!(
            output.contains("⚠ keyjey.context_window.exceeds_200k"),
            "expected '⚠ keyjey.context_window.exceeds_200k' in output: {output}"
        );
    }
    ...
    fn test_error_hint_for_disabled_module_returns_disabled_text() {
        ...
        let (error, remediation) = error_hint_for("keyjey.model", &ctx, &cfg);
        assert!(
            error.contains("disabled"),
            "expected 'disabled' in error hint: {error}"
        );
        assert!(
            remediation.contains("[keyjey.model]"),
            "expected specific section '[keyjey.model]' in remediation: {remediation}"
        );
    }
