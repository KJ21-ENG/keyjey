//! File-based cache for keyjey module results.
//!
//! ## Passthrough cache (Story 4.2)
//! Path: `{dirname(transcript_path)}/keyjey/{transcript_stem}-starship-{module_name}`
//! TTL: 5 seconds via file mtime. Format: raw UTF-8 text.
//!
//! ## Usage limits cache (Story 5.2)
//! Path: `{dirname(transcript_path)}/keyjey/{transcript_stem}-usage-limits`
...
/// Example: `.../session.jsonl` → `.../keyjey/session-usage-limits`
fn usage_limits_cache_path(transcript_path: &Path) -> Option<std::path::PathBuf> {
    let dir = transcript_path.parent()?;
    let stem = transcript_path.file_stem()?.to_str()?;
    Some(dir.join("keyjey").join(format!("{stem}-usage-limits")))
}
...
mod tests {
    ...
    fn test_usage_limits_cache_file_path_and_json_structure() {
        ...
        let transcript = dir.path().join("transcript.jsonl");
        write_usage_limits(&transcript, &sample_data(), 60);
        let expected_path = dir.path().join("keyjey").join("transcript-usage-limits");
    ...
    fn test_usage_limits_ttl_invalidation() {
        ...
        let path = dir.path().join("keyjey").join("transcript-usage-limits");
