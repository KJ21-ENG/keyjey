/// Render the `[keyjey.model]` module.
///
...
pub fn render(ctx: &crate::context::Context, cfg: &crate::config::KeyjeyConfig) -> Option<String> {
    ...
    let display_name = match ctx.model.as_ref().and_then(|m| m.display_name.as_deref()) {
        Some(name) => name,
        None => {
            tracing::warn!("keyjey.model: model.display_name is absent — skipping");
            return None;
        }
    };
...
/// Renders `$keyjey.model.display_name` — explicit sub-field alias for `render`.
...
/// Renders `$keyjey.model.id` — raw model ID string (e.g., "claude-opus-4-6").
pub fn render_id(
    ctx: &crate::context::Context,
    cfg: &crate::config::KeyjeyConfig,
) -> Option<String> {
    ...
    let id = match ctx.model.as_ref().and_then(|m| m.id.as_deref()) {
        Some(id) => id,
        None => {
            tracing::warn!("keyjey.model: model.id is absent — skipping");
            return None;
        }
    };
    let symbol = model_cfg.and_then(|m| m.symbol.as_deref());
...
mod tests {
    ...
    fn test_render_id_returns_model_id() {
        ...
        assert_eq!(
            render_id(&ctx, &KeyjeyConfig::default()),
    ...
    fn test_render_id_absent_returns_none() {
        let ctx = Context::default();
        assert_eq!(render_id(&ctx, &KeyjeyConfig::default()), None);
    }
