...
/// Renders `$keyjey.context_bar` — visual Unicode progress bar with threshold color escalation.
/// Format: `{bar}{used_percentage:.0}%` e.g. `███░░░░░░░35%`
/// Bar width is configurable via `[keyjey.context_bar].width` (default 10).
///
...
mod tests {
    use super::*;
    use crate::config::{ContextBarConfig, KeyjeyConfig};
