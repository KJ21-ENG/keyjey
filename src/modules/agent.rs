/// Render the `[keyjey.agent]` module.
///
/// `$keyjey.agent` — convenience alias for `$keyjey.agent.name`.
/// `$keyjey.agent.name` — raw agent name string (e.g., "claude-code", "security-reviewer").
///
/// [Source: epics.md#Story 2.3, architecture.md#Module System Architecture]
use crate::config::KeyjeyConfig;
use crate::context::Context;

/// Renders `$keyjey.agent` — convenience alias for agent name.
pub fn render(ctx: &Context, cfg: &KeyjeyConfig) -> Option<String> {
    render_name(ctx, cfg)
}

/// Renders `$keyjey.agent.name` — raw agent name string with optional symbol and style.
pub fn render_name(ctx: &Context, cfg: &KeyjeyConfig) -> Option<String> {
    ...
    let name = match ctx.agent.as_ref().and_then(|a| a.name.as_ref()) {
        Some(n) => n,
        None => {
            tracing::warn!("keyjey.agent: name absent from context");
            return None;
        }
    };
...
mod tests {
    use super::*;
    use crate::config::{AgentConfig, KeyjeyConfig};
    ...
    fn test_agent_name_alias_identical_to_render() {
        ...
        let r1 = render(&ctx, &KeyjeyConfig::default());
        let r2 = render_name(&ctx, &KeyjeyConfig::default());
        assert_eq!(r1, r2);
    }
    ...
    fn test_agent_absent_returns_none() {
        ...
        assert_eq!(render(&ctx, &KeyjeyConfig::default()), None);
    }
