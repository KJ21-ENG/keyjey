...
use crate::cache;
use crate::config::{KeyjeyConfig, UsageLimitsConfig};
...
pub fn render(ctx: &Context, cfg: &KeyjeyConfig) -> Option<String> {
    ...
    // keyjey is invoked outside a Claude Code session (not an error condition).
    ...
    let data = if let Some(cached) = cache::read_usage_limits(transcript_path, false) {
        ...
        let token = match crate::platform::get_oauth_token() {
            Ok(t) => t,
            Err(e) => {
                tracing::warn!("keyjey.usage_limits: credential retrieval failed: {e}");
                return None;
            }
        };
    ...
    F: FnOnce() -> Result<UsageLimitsData, String> + Send + 'static,
{
    ...
    match rx.recv_timeout(std::time::Duration::from_secs(2)) {
        ...
        Ok(Err(e)) => {
            tracing::warn!("keyjey.usage_limits: API fetch failed: {e}");
            None
        }
        Err(_) => {
            tracing::warn!("keyjey.usage_limits: API fetch timed out after 2s");
            None
        }
    }
}
