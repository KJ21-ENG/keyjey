...
/// Renders `$keyjey.workspace.current_dir` — workspace current directory from Context.
pub fn render_current_dir(
        ...
        .and_then(|w| w.current_dir.as_deref())
    {
        Some(v) => v,
        None => {
            tracing::warn!("keyjey.workspace.current_dir: field absent from context");
            return None;
        }
    };
...
/// Renders `$keyjey.workspace.project_dir` — workspace project directory from Context.
pub fn render_project_dir(
        ...
        .and_then(|w| w.project_dir.as_deref())
    {
        Some(v) => v,
        None => {
            tracing::warn!("keyjey.workspace.project_dir: field absent from context");
            return None;
        }
    };
    let ws_cfg = cfg.workspace.as_ref();
...
mod tests {
    use super::*;
    use crate::config::{KeyjeyConfig, WorkspaceConfig};
    ...
    fn test_render_current_dir_renders_value() {
        ...
        let result = render_current_dir(&ctx, &KeyjeyConfig::default());
    ...
    fn test_render_current_dir_absent_returns_none() {
        ...
        assert_eq!(render_current_dir(&ctx, &KeyjeyConfig::default()), None);
    }
    ...
    fn test_render_project_dir_renders_value() {
        let ctx = ctx_with_workspace("/home/user/projects/myapp", "/home/user/projects/myapp");
        let result = render_project_dir(&ctx, &KeyjeyConfig::default());
        assert_eq!(result, Some("/home/user/projects/myapp".to_string()));
    }
    ...
    fn test_render_project_dir_absent_returns_none() {
        let ctx = Context::default();
        assert_eq!(render_project_dir(&ctx, &KeyjeyConfig::default()), None);
    }
