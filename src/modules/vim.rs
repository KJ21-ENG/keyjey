/// Render the `[keyjey.vim]` module.
///
/// `$keyjey.vim` — convenience alias for `$keyjey.vim.mode`.
/// `$keyjey.vim.mode` — raw vim mode string (e.g., "NORMAL", "INSERT").
///
/// [Source: epics.md#Story 2.3, architecture.md#Module System Architecture]
use crate::config::KeyjeyConfig;
use crate::context::Context;

/// Renders `$keyjey.vim` — convenience alias for vim mode.
pub fn render(ctx: &Context, cfg: &KeyjeyConfig) -> Option<String> {
    render_mode(ctx, cfg)
}

/// Renders `$keyjey.vim.mode` — raw vim mode string with optional symbol and style.
pub fn render_mode(ctx: &Context, cfg: &KeyjeyConfig) -> Option<String> {
    ...
    let mode = match ctx.vim.as_ref().and_then(|v| v.mode.as_ref()) {
        Some(m) => m,
        None => {
            tracing::warn!("keyjey.vim: mode absent from context");
            return None;
        }
    };
...
mod tests {
    use super::*;
    use crate::config::{KeyjeyConfig, VimConfig};
    ...
    fn test_vim_mode_alias_identical_to_render() {
        ...
        let r2 = render_mode(&ctx, &KeyjeyConfig::default());
        assert_eq!(r1, r2);
    }
