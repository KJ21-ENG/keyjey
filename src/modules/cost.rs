/// Render the `[keyjey.cost]` family of modules.
///
/// `$keyjey.cost` — convenience alias: formats total_cost_usd as "$X.XX" with threshold styling.
/// `$keyjey.cost.total_cost_usd` — raw USD value, 4 decimal places.
/// `$keyjey.cost.total_duration_ms` / `total_api_duration_ms` — integer milliseconds.
/// `$keyjey.cost.total_lines_added` / `total_lines_removed` — integer counts.
///
/// [Source: epics.md#Story 2.1, architecture.md#Structure Patterns]
use crate::config::{CostConfig, CostSubfieldConfig, KeyjeyConfig};
use crate::context::Context;

/// Renders `$keyjey.cost` — total cost as `$X.XX` with threshold color escalation.
pub fn render(ctx: &Context, cfg: &KeyjeyConfig) -> Option<String> {
    ...
    let val = match ctx.cost.as_ref().and_then(|c| c.total_cost_usd) {
        Some(v) => v,
        None => {
            tracing::warn!("keyjey.cost: total_cost_usd absent from context");
            return None;
        }
    };
...
/// Renders `$keyjey.cost.total_cost_usd` — raw USD value to 4 decimal places.
pub fn render_total_cost_usd(ctx: &Context, cfg: &KeyjeyConfig) -> Option<String> {
    let cost_cfg = cfg.cost.as_ref();
    ...
    let val = match ctx.cost.as_ref().and_then(|c| c.total_cost_usd) {
        Some(v) => v,
        None => {
            tracing::warn!("keyjey.cost.total_cost_usd: value absent from context");
            return None;
        }
    };
...
/// Renders `$keyjey.cost.total_duration_ms` — total wall time in milliseconds.
pub fn render_total_duration_ms(ctx: &Context, cfg: &KeyjeyConfig) -> Option<String> {
    let cost_cfg = cfg.cost.as_ref();
    ...
    let val = match ctx.cost.as_ref().and_then(|c| c.total_duration_ms) {
        Some(v) => v,
        None => {
            tracing::warn!("keyjey.cost.total_duration_ms: value absent from context");
            return None;
        }
    };
...
/// Renders `$keyjey.cost.total_api_duration_ms` — API-only duration in milliseconds.
pub fn render_total_api_duration_ms(ctx: &Context, cfg: &KeyjeyConfig) -> Option<String> {
    let cost_cfg = cfg.cost.as_ref();
    ...
    let val = match ctx.cost.as_ref().and_then(|c| c.total_api_duration_ms) {
        Some(v) => v,
        None => {
            tracing::warn!("keyjey.cost.total_api_duration_ms: value absent from context");
            return None;
        }
    };
    let val_str = val.to_string();
...
/// Renders `$keyjey.cost.total_lines_added` — cumulative lines added this session.
pub fn render_total_lines_added(ctx: &Context, cfg: &KeyjeyConfig) -> Option<String> {
    let cost_cfg = cfg.cost.as_ref();
    ...
    let val = match ctx.cost.as_ref().and_then(|c| c.total_lines_added) {
        Some(v) => v,
        None => {
            tracing::warn!("keyjey.cost.total_lines_added: value absent from context");
            return None;
        }
    };
    let val_str = val.to_string();
...
/// Renders `$keyjey.cost.total_lines_removed` — cumulative lines removed this session.
pub fn render_total_lines_removed(ctx: &Context, cfg: &KeyjeyConfig) -> Option<String> {
    let cost_cfg = cfg.cost.as_ref();
    ...
    let val = match ctx.cost.as_ref().and_then(|c| c.total_lines_removed) {
        Some(v) => v,
        None => {
            tracing::warn!("keyjey.cost.total_lines_removed: value absent from context");
            return None;
        }
    };
    let val_str = val.to_string();
...
mod tests {
    use super::*;
    use crate::config::{CostConfig, KeyjeyConfig};
    ...
    fn test_cost_renders_dollar_formatted() {
        ...
        let result = render(&ctx, &KeyjeyConfig::default());
    ...
    fn test_render_total_cost_usd_four_decimal_places() {
        let ctx = ctx_with_cost(0.01234);
        let result = render_total_cost_usd(&ctx, &KeyjeyConfig::default());
    ...
    fn test_render_total_duration_ms() {
        ...
        let result = render_total_duration_ms(&ctx, &KeyjeyConfig::default());
    ...
    fn test_render_total_api_duration_ms() {
        let ctx = ctx_with_cost(0.01);
        let result = render_total_api_duration_ms(&ctx, &KeyjeyConfig::default());
    ...
    fn test_render_total_lines_added() {
        let ctx = ctx_with_cost(0.01);
        let result = render_total_lines_added(&ctx, &KeyjeyConfig::default());
    ...
    fn test_render_total_lines_removed() {
        let ctx = ctx_with_cost(0.01);
        let result = render_total_lines_removed(&ctx, &KeyjeyConfig::default());
    ...
    fn test_subfield_no_threshold_unchanged() {
        ...
        let ctx = ctx_with_cost(0.01234);
        let result_default = render_total_cost_usd(&ctx, &KeyjeyConfig::default());
