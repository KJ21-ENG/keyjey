...
/// Renders `$keyjey.cwd` — current working directory from Context.
pub fn render_cwd(
    ...
    let value = match ctx.cwd.as_deref() {
        Some(v) => v,
        None => {
            tracing::warn!("keyjey.cwd: field absent from context");
            return None;
        }
    };
...
/// Renders `$keyjey.session_id` — session ID string from Context.
pub fn render_session_id(
    ...
    let value = match ctx.session_id.as_deref() {
        Some(v) => v,
        None => {
            tracing::warn!("keyjey.session_id: field absent from context");
            return None;
        }
    };
    let sess_cfg = cfg.session.as_ref();
...
/// Renders `$keyjey.transcript_path` — transcript file path from Context.
pub fn render_transcript_path(
    ...
    let value = match ctx.transcript_path.as_deref() {
        Some(v) => v,
        None => {
            tracing::warn!("keyjey.transcript_path: field absent from context");
            return None;
        }
    };
    let sess_cfg = cfg.session.as_ref();
...
/// Renders `$keyjey.version` — Claude Code version string from Context.
pub fn render_version(
    ...
    let value = match ctx.version.as_deref() {
        Some(v) => v,
        None => {
            tracing::warn!("keyjey.version: field absent from context");
            return None;
        }
    };
    let sess_cfg = cfg.session.as_ref();
...
/// Renders `$keyjey.output_style` — output style name from Context (nested field).
pub fn render_output_style(
    ...
    let value = match ctx.output_style.as_ref().and_then(|o| o.name.as_deref()) {
        Some(v) => v,
        None => {
            tracing::warn!("keyjey.output_style: field absent from context");
            return None;
        }
    };
    let sess_cfg = cfg.session.as_ref();
...
mod tests {
    use super::*;
    use crate::config::{KeyjeyConfig, SessionConfig};
    ...
    fn test_render_cwd_renders_value() {
        let ctx = ctx_with_cwd("/home/leo/projects/keyjey");
        let result = render_cwd(&ctx, &KeyjeyConfig::default());
        assert_eq!(result, Some("/home/leo/projects/keyjey".to_string()));
    }
    ...
    fn test_render_cwd_absent_returns_none() {
        ...
        assert_eq!(render_cwd(&ctx, &KeyjeyConfig::default()), None);
    }
    ...
    fn test_render_session_id_renders_value() {
        ...
        let result = render_session_id(&ctx, &KeyjeyConfig::default());
    ...
    fn test_render_session_id_absent_returns_none() {
        let ctx = Context::default();
        assert_eq!(render_session_id(&ctx, &KeyjeyConfig::default()), None);
    }
    ...
    fn test_render_transcript_path_renders_value() {
        ...
        let result = render_transcript_path(&ctx, &KeyjeyConfig::default());
        assert_eq!(
            result,
    ...
    fn test_render_transcript_path_absent_returns_none() {
        let ctx = Context::default();
        assert_eq!(render_transcript_path(&ctx, &KeyjeyConfig::default()), None);
    }
    ...
    fn test_render_version_renders_value() {
        ...
        let result = render_version(&ctx, &KeyjeyConfig::default());
    ...
    fn test_render_version_absent_returns_none() {
        let ctx = Context::default();
        assert_eq!(render_version(&ctx, &KeyjeyConfig::default()), None);
    }
    ...
    fn test_render_output_style_renders_name() {
        ...
        let result = render_output_style(&ctx, &KeyjeyConfig::default());
    ...
    fn test_render_output_style_absent_returns_none() {
        let ctx = Context::default();
        assert_eq!(render_output_style(&ctx, &KeyjeyConfig::default()), None);
    }
